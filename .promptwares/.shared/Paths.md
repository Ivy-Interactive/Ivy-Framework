# Folders and Important Files

> **Note:** If a Project is specified in the firmware header (not `[Auto]` or `General`), prefer the `project-context.md` file in the PlanFolder over this general reference file. The project-context.md file contains targeted context for the specific project.

## Ivy-Framework

D:\Repos\_Ivy\Ivy-Framework\

Basic instructions about Ivy Framework:

D:\Repos\_Ivy\Ivy-Framework\AGENTS.md

Docs:

D:\Repos\_Ivy\Ivy-Framework\src\Ivy.Docs.Shared\Docs

Samples:

D:\Repos\_Ivy\Ivy-Framework\src\Ivy.Samples.Shared\

Breaking changes in Ivy-Framework are documented as "refactor" prompts:

D:\Repos\_Ivy\Ivy-Framework\src\.releases\Refactors

## Agent Clients

D:\Repos\_Ivy\Ivy\Ivy.Console\
D:\Repos\_Ivy\Ivy\Ivy.Internals\

## Agent Server

D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent.Server
D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent

Prompt for the Developer persona (default):
D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent\Agents\Personas\Prompts\Developer.md

## Shared

Shared code between client and server, including message definitions.

D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent.Shared

The client and server communicates over websockets and sends "messages". All messages in the system are
defined in:

D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent.Shared\Messages

## Inspectors

D:\Repos\_Ivy\Ivy-Inspectors

## MCP

This is the service that we call for IvyQuestions and IvyDocs.

D:\Repos\_Ivy\Ivy-Mcp

## Workflows

Agent workflow definitions are defined in:

D:\Repos\_Ivy\Ivy\Ivy.Internals\Workflows

Read more about how workflows are created in:

D:\Repos\_Ivy\Ivy\Ivy.Internals\Workflows\Internal\Workflows\CreateOrEditWorkflowWorkflow.workflow

# Reference Connections

D:\Repos\_Ivy\Ivy\connections

# Scripts

My personal scripts are stored under:

D:\Repos\_Personal\Scripts\
D:\Repos\_Personal\Scripts\AF2\

When refering to scripts this is almost always where they are stored.

# Vscode Extension

D:\Repos\_Personal\Scripts\AF2\.vscode-extensions

# Test Manager

 D:\Repos\_Ivy\Ivy-Agent\Ivy.Agent.Test.Manager

 # Tendril 

Tools for managing our plan and harness engineering. 

 D:\Repos\_Ivy\Ivy-Tendril\

