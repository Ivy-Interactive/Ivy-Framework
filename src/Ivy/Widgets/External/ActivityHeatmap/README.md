...
3. Build the widget (repository root, or any path that builds `src/Ivy/Ivy.csproj`):

   ```bash
   dotnet build
   ```

   `src/Ivy/Ivy.csproj` runs `vp install` / `vp build` in this widget’s `frontend` when needed, so after dependencies exist you can rely on `dotnet build` instead of repeating steps 1–2.
