...
See `src/Ivy/Widgets/External/Xterm/.samples/Apps/OutputCaptureApp.cs` for a full sample that shows a live `Terminal` next to a read-only pane rendering `AnsiEscape.Strip(pty.Output)`, refreshed on a `UseInterval` poll.
