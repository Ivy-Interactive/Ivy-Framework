...
public class GridLayoutTestApp : ViewBase
{
    public override object? Build()
    {
        return Layout.Vertical()
            ...
            | Text.H2("GridRowSpan (button spanning 2 rows)")
            | Text.P("The = button spans rows 3–4")
            | (Layout.Grid()
                .Columns(4)
                .Rows(5)
                .Gap(2)
                ...
                | new Button("1") | new Button("2") | new Button("3") | new Button("=").GridRowSpan(2)
                | new Button("0").GridColumnSpan(2) | new Button(".")
              )

            | Text.H2("GridColumnSpan (header spanning full width)")
            | Text.P("Header spans 3 columns, sidebar spans 2 rows")
            | (Layout.Grid()
                .Columns(3)
                .Rows(3)
                .Gap(2)
                .Height(Size.Px(200))
                | "Header".WithCell().Background(Colors.Blue).GridColumnSpan(3)
                | "Sidebar".WithCell().Background(Colors.Green).GridRowSpan(2)
                | "Main".WithCell().Background(Colors.Orange)
                | "Aside".WithCell().Background(Colors.Rose)
                | "Footer 1".WithCell()
                | "Footer 2".WithCell()
              )
