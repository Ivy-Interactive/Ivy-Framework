    ...
    protected override object? BuildSample()
    {
        ...
        return Layout.Vertical(
            Layout.Horizontal(
                ...
                new Button("Destructive Toast", _ => client.Toast("An error occurred during the operation.", "Error", ToastVariant.Destructive)),
                new Button("Success Toast", _ => client.Toast("Record saved successfully.", "Success", ToastVariant.Success)),
                new Button("Warning Toast", _ => client.Toast("Your session is about to expire.", "Warning", ToastVariant.Warning)),
                new Button("Info Toast", _ => client.Toast("A new update is available for download.", "Info", ToastVariant.Info))
            ).Wrap(),
            ...
            Layout.Horizontal(
                ...
                new Button("No Title", _ => client.Toast("This toast has no title, only a description.", variant: ToastVariant.Warning)),
                new Button("Exception Toast", _ =>
                {
                    ...
                    {
                        client.Toast(ex, ToastVariant.Destructive);
                    }
                })
            ).Wrap()
        ).Gap(16);
