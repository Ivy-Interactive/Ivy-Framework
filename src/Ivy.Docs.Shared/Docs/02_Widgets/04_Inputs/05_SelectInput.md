...
For string options, use either pattern: `state.ToSelectInput(["A", "B", ...])` or `state.ToSelectInput().Options(["A", "B", ...])`. Use [Size](../../04_ApiReference/IvyShared/Size.md) for `.Width(Size.Full())` to make the select fill available space:
