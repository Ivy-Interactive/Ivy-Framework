...
## ListItem.Chevron — non-existent property

**Hallucinated API:**
```csharp
new ListItem("Navigate").Chevron()
```

**Error:** `CS1061: 'ListItem' does not contain a definition for 'Chevron'`

**Correct API:**
```csharp
new ListItem("Navigate").WithIcon(Icons.ChevronRight)
```

Use `WithIcon(Icons.ChevronRight)` for navigation indicators on list items.

**Found In:**
8e25c118 (Build #4)

## Table.Column — non-existent static method

**Hallucinated API:**
```csharp
Table.Column(t => t.Name, "Name")
```

**Error:** `CS0117: 'Table' does not contain a definition for 'Column'`

**Correct API:**
```csharp
// Use DataTableBuilder.Header() to define columns:
items.ToDataTable()
    .Header(t => t.Name, "Name")
    .Header(t => t.Email, "Email")
```

`Table` is a layout widget. For defining DataTable columns, use the `.Header()` method on `DataTableBuilder<T>`.

**Found In:**
8e25c118 (Build #4)

## DataTableBuilder<T>.OnRowClick — non-existent event handler
...
**Error:** `CS1061: 'DataTableBuilder<Tenant>' does not contain a definition for 'OnRowClick'`

**Correct API:**
```csharp
// Use OnCellClick and access row via index:
var items = db.Tenants.ToList();
items.AsQueryable().ToDataTable()
    .OnCellClick((e) => {
        var tenant = items[e.Value.RowIndex];
        blades.Open(new TenantDetailsBlade(tenant.Id));
    })
```

There is no `OnRowClick` method on `DataTableBuilder<T>`. Use `OnCellClick` and access the clicked row via `RowIndex`.

**Found In:**
8e25c118 (Build #16)

## Event<DataTable, CellClickEventArgs>.Row — non-existent property
...
**Error:** `CS1061: 'Event<DataTable, CellClickEventArgs>' does not contain a definition for 'Row'`

**Correct API:**
```csharp
// Access via RowIndex from your original data source:
var items = db.Tenants.ToList();
items.AsQueryable().ToDataTable()
    .OnCellClick((e) => {
        var tenant = items[e.Value.RowIndex];
...
`CellClickEventArgs` provides `RowIndex`, `ColumnIndex`, `ColumnName`, and `CellValue`. There is NO `Row` property. Use `RowIndex` to access the row from your data source.

**Found In:**
8e25c118 (Builds #13, #17)

## Event<DataTable, CellClickEventArgs>.RowIndex direct access — incorrect event structure
...
**Error:** `CS1061: 'Event<DataTable, CellClickEventArgs>' does not contain a definition for 'RowIndex'`

**Correct API:**
```csharp
.OnCellClick((e) => {
    var index = e.Value.RowIndex; // Access via .Value
})
```

Event handlers receive an `Event<TWidget, TArgs>` wrapper. Access event properties via `e.Value.PropertyName`, not directly on `e`.

**Found In:**
8e25c118 (Build #18)

## Skeleton.Table() — non-existent loading skeleton
...
**Error:** `CS1061: 'Skeleton' does not contain a definition for 'Table'`

**Correct API:**
```csharp
// Use Skeleton.Card() for loading states:
return Skeleton.Card();

// Or use multiple skeleton elements:
...
There is no `Skeleton.Table()` method. Use `Skeleton.Card()` or compose multiple `Skeleton.Line()` elements for table loading states.

**Found In:**
8e25c118 (Build #12)

## QueryOptions.OnSuccess — non-existent callback property
...
**Error:** `CS0117: 'QueryOptions' does not contain a definition for 'OnSuccess'`

**Correct API:**
```csharp
// Handle success logic inside your mutation function:
...
`QueryOptions` has `RefreshInterval`, `LifetimeType`, and `Tags` properties. There is NO `OnSuccess` callback. Handle success in your mutation function directly.

**Found In:**
8e25c118 (Build #12)

## MutationOptions<TInput, TOutput> — non-existent generic type

**Hallucinated API:**
```csharp
var mutation = this.UseMutation<CreateTenantInput, Tenant>(async (input) => {
    return await tenantService.CreateAsync(input);
});
```

**Error:** `CS0246: The type or namespace name 'MutationOptions<,>' could not be found` or `CS0308: The non-generic method 'UseMutation' cannot be used with type arguments`

**Correct API:**
```csharp
// Use non-generic UseMutation:
var mutation = this.UseMutation(async () => {
    var input = new CreateTenantInput { Name = nameState.Get() };
    await tenantService.CreateAsync(input);
});
```

There is NO generic `UseMutation<TInput, TOutput>` or `MutationOptions<,>`. The method signature is:
```csharp
UseMutation(Func<Task> mutation, QueryOptions? options = null)
```

**Found In:**
8e25c118 (Builds #9, #11)

## Ivy.Forms namespace — non-existent namespace
...
**Error:** `CS0234: The type or namespace name 'Forms' does not exist in the namespace 'Ivy'`

**Correct API:**
```csharp
// All types are in the Ivy namespace (global usings handle this):
// No using statement needed for Ivy types

// Or explicitly:
using Ivy;
```

There is no `Ivy.Forms` namespace. All Ivy types (including form-related types) are directly in the `Ivy` namespace. Global usings typically handle this automatically.

**Found In:**
8e25c118 (Build #10)

## Ivy.Components namespace — non-existent namespace
...
**Error:** `CS0234: The type or namespace name 'Components' does not exist in the namespace 'Ivy'`
...
There is no `Ivy.Components` namespace. All Ivy types (widgets, views, layouts) are directly in the `Ivy` namespace. Global usings typically handle this automatically.
