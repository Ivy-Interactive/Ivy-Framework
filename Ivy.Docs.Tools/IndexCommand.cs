...
public class IndexCommand : Command<IndexCommand.Settings>
{
    public class Settings : CommandSettings
    {
        ...
        public string OutputPath { get; set; } = "documentation-index.json";
        
        [CommandOption("-p|--path")]
        public string DocsPath { get; set; } = Path.Combine(Directory.GetCurrentDirectory(), "Ivy.Docs", "Docs");
    ...
    public override int Execute(CommandContext context, Settings settings)
    {
        ...
        {
            AnsiConsole.MarkupLine($"[red]❌ Documentation directory not found: {settings.DocsPath}[/]");
            return 1;
        }

        var indexer = new DocumentationIndexer(settings.DocsPath);
        ...
        indexer.SaveIndex(settings.OutputPath);
        
        AnsiConsole.MarkupLine($"[green]✅ Documentation indexing completed![/]");
        var totalFiles = Directory.GetFiles(settings.DocsPath, "*.md", SearchOption.AllDirectories).Length;
        AnsiConsole.MarkupLine($"[blue]📊 Total documents indexed: {totalFiles}[/]");
...
public class DocumentationIndexer
{
    ...
    {
        AnsiConsole.MarkupLine("[yellow]📚 Generating documentation index...[/]");
        ...
        AnsiConsole.MarkupLine($"[green]✅ Generated index for {_index.Count} documents[/]");
    }
    ...
    public void SaveIndex(string outputPath)
    {
        ...
        AnsiConsole.MarkupLine($"[green]💾 Index saved to {outputPath}[/]");
    }

    public void PrintIndex()
    {
        AnsiConsole.MarkupLine("\n[bold blue]📋 DOCUMENTATION INDEX[/]\n");
        ...
        {
            AnsiConsole.MarkupLine($"\n[bold green]## {group.Key}[/]");
            AnsiConsole.WriteLine(new string('-', 40));
            
            foreach (var doc in group.OrderBy(d => d.Title))
            {
                AnsiConsole.MarkupLine($"\n[bold yellow]### {doc.Title}[/]");
                AnsiConsole.MarkupLine($"[dim]📄 File: {doc.FilePath}[/]");
                AnsiConsole.MarkupLine($"[cyan]🤔 Questions ({doc.Questions.Count}):[/]");
                
                foreach (var question in doc.Questions)
                {
                    AnsiConsole.MarkupLine($"  [dim]•[/] {question}");
                }
            }
        }
    }
