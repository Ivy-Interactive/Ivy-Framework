...
export const TabWidget: React.FC<TabWidgetProps> = React.memo(
  ({ children }) => {
...
const SortableTabTrigger = React.memo(function SortableTabTrigger({
  id,
  value,
  onClick,
  onMouseDown,
...
const SortableDropdownMenuItem = React.memo(function SortableDropdownMenuItem({
  id,
  children,
  onClick,
  isActive,
...
export const TabsLayoutWidget = React.memo(
  ({
    id,
    children,
    events,
    ...
    // Create a stable content cache that preserves tab content across children changes
    const tabContentCache = React.useRef<Map<string, React.ReactElement>>(
      new Map()
    );

    // Memoize tabWidgets and maintain stable content references
    const tabWidgets = React.useMemo(() => {
      const newTabWidgets = React.Children.toArray(children).filter(
        child =>
      ...
      // Update cache with new content, but preserve existing instances where possible
      newTabWidgets.forEach(tabWidget => {
        const tabId = tabWidget.props.id;
        const existingTab = tabContentCache.current.get(tabId);

        // Only update if the content actually changed (deep comparison of props)
        if (
          !existingTab ||
          (existingTab.props as TabWidgetProps).title !==
            tabWidget.props.title ||
          (existingTab.props as TabWidgetProps).badge !==
            tabWidget.props.badge ||
          (existingTab.props as TabWidgetProps).icon !== tabWidget.props.icon
        ) {
          tabContentCache.current.set(tabId, tabWidget);
        }
      });

      // Clean up cache for removed tabs
      const newTabIds = new Set(newTabWidgets.map(tab => tab.props.id));
      for (const [tabId] of tabContentCache.current) {
        if (!newTabIds.has(tabId)) {
          tabContentCache.current.delete(tabId);
        }
      }

      // Return stable references from cache
      return newTabWidgets.map(
        tabWidget =>
          tabContentCache.current.get(tabWidget.props.id) || tabWidget
      );
    }, [children]);
      ...
      try {
        ...
        for (const tabId of tabOrder) {
          const tabWidget = tabWidgetsRef.current.find(
            tab =>
              (tab as React.ReactElement<TabWidgetProps>).props.id === tabId
          );
    ...
    const debouncedCalculateVisibleTabs = useDebounce(
    ...
    const handleDragEnd = React.useCallback(
      (event: {
        ...
        if (active && over && active.id !== over.id) {
          ...
          const oldActiveIndex = activeTabId
            ? tabOrder.indexOf(activeTabId)
            : -1;
    ...
    // Create a stable map of tab widgets by ID to preserve React element identity for headers
    const tabWidgetMap = React.useMemo(() => {
      const map = new Map<string, React.ReactElement<TabWidgetProps>>();
      tabWidgets.forEach(tab => {
        if (React.isValidElement(tab)) {
          const typedTab = tab as React.ReactElement<TabWidgetProps>;
          map.set(typedTab.props.id, typedTab);
        }
      });
      return map;
    }, [tabWidgets]);

    // Only used for rendering tab headers in the correct order
    const orderedTabWidgets = React.useMemo(
      () =>
        tabOrder
          .map(id => tabWidgetMap.get(id))
          .filter(Boolean) as React.ReactElement<TabWidgetProps>[],
      [tabOrder, tabWidgetMap]
    );
    ...
    const renderTabContent = React.useCallback(
      (tabWidget: React.ReactElement) => {
        ...
        const {
          ...
          icon,
          badge,
        } = tabWidget.props as TabWidgetProps;
      ...
      [activeTabId, showRefresh, showClose, eventHandler, id, tabOrder]
    );
