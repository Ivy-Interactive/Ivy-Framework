...
{
    if (condition) {
        var count = UseState(0); // Might not run
    }
    var name = UseState(""); // Order changes!
    return Layout.Vertical();
}
```

### 2. Extract Logic, Not Hooks

```csharp
// Good: Extract logic, keep hooks at top level
public override object Build()
{
    var data = UseState(0);
    var processed = ProcessData(data.Value); // Logic extracted
    return Layout.Vertical();
}

// Bad: Trying to extract hook
private void SetupState()
{
    var data = UseState(0); // Error! Not in Build method
}
```
...
private (IAnyState<int> count, Action increment) UseCounter(int initial = 0)
{
    var count = UseState(initial);
    var increment = UseCallback(() => count.Set(count.Value + 1), count);
    return (count, increment);
}

public override object Build()
{
    var (count, increment) = UseCounter(0); // Reusable hook
    return Layout.Vertical(
        Text.P($"Count: {count.Value}"),
        new Button("+", _ => increment())
    );
}
```
