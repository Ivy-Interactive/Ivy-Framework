...
This example demonstrates how to properly manage [state](../../04_Hooks/03_State.md) with observables by controlling when subscriptions are active. It shows a timer-based counter that only increments when a state flag is active, with proper UI updates and subscription cleanup.
