...
<Ingress>
Create dropdown selectors that load options asynchronously from [APIs](../../01_Onboarding/02_Concepts/Services.md) or [databases](../../01_Onboarding/02_Concepts/Program.md), perfect for large datasets and on-demand loading.
</Ingress>

The `AsyncSelectInput` [widget](../../01_Onboarding/02_Concepts/Widgets.md) provides a select dropdown that loads options asynchronously. It's useful for scenarios where options need to be fetched from an [API](../../01_Onboarding/02_Concepts/Services.md) or when the list of options is large and should be loaded on-demand.
...
<Callout Type="tip">
AsyncSelectInput automatically handles loading states and provides a smooth user [experience](../../01_Onboarding/02_Concepts/Views.md). The query function is called as the user types, and the lookup function is called when displaying the selected value.
</Callout>
