...
2. **Обработка путей**
   - Использовать директорию `Generated/` как корневую для `.llm.md` файлов
   - Создавать `.llm.md` в том же каталоге, где находится соответствующий `.g.cs` файл (например, для виджетов — в `Generated/Widgets/...`)
   - Сохранять структуру директорий в пределах `Generated/`
